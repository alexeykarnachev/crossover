            ...
            switch (handle.tag) {
                ...
                case TRANSFORMATION_ORIENTATION_HANDLE: {
                    ...
                    if (fabs(diff) > EPS) {
                        ...
                        update_orientation(
                            entity, transformation->curr_orientation - diff
                        );
                    }
                    break;
                }
                ...
                case LINE_VERTEX_HANDLE: {
                    if (fabs(rot_diff.x) > EPS || fabs(rot_diff.y) > EPS) {
                        apply_orient_dependent_drag = 1;
                        primitive->p.line.b = add(
                            primitive->p.line.b, scale(rot_diff, 2.0)
                        );
                    }
                    break;
                }
