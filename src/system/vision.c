...
static int get_view_rays(Vision vision, float orientation, Vec2 *out) {
    Vec2 origin = {0.0, 0.0};
    Vec2 start_ray = rotate(vec2(vision.distance, 0.0), origin, orientation);
...
void update_visions() {
    ...
    for (int entity = 0; entity < SCENE.n_entities; ++entity) {
        ...
        int n_view_rays = get_view_rays(*vision, t0.curr_orientation, VIEW_RAYS_ARENA);
    ...
    for (int entity = 0; entity < SCENE.n_entities; ++entity) {
        ...
        for (int i = 0; i < n_view_rays; ++i) {
            ...
            if (observation.entity != -1) {
                ...
                render_debug_circle(
                    observation.position, 0.05, RED_COLOR, DEBUG_RENDER_LAYER, FILL
                );
            } else {
                render_debug_line(
                    t0.curr_position,
