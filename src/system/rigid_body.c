...
void update_rigid_bodies(float dt) {
    for (int entity = 0; entity < SCENE.n_entities; ++entity) {
        ...
        switch (rb->type) {
            case STATIC_RIGID_BODY: continue;
            ...
            case DYNAMIC_RIGID_BODY: {
                ...
                Vec2 damping_force = scale(
                    rb->b.dynamic_rb.linear_velocity, -rb->b.dynamic_rb.linear_damping
                );
                Vec2 net_force = add(rb->b.dynamic_rb.net_force, damping_force);
                Vec2 linear_acceleration = scale(net_force, 1.0f / rb->b.dynamic_rb.mass);
                rb->b.dynamic_rb.linear_velocity = add(
                    rb->b.dynamic_rb.linear_velocity, scale(linear_acceleration, dt)
                );
                ...
                float net_torque = rb->b.dynamic_rb.net_torque + damping_torque;
                float angular_acceleration = net_torque
                                             / rb->b.dynamic_rb.moment_of_inertia;
                rb->b.dynamic_rb.angular_velocity += angular_acceleration * dt;
            ...
            default: {
                fprintf(
                    stderr, "ERROR: Unknown rigid body type: %d. It's a bug\n", rb->type
                );
            }
        }
        ...
        float orientation = transformation->curr_orientation + angular_step;
